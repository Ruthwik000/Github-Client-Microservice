...
export class RAGService {
    ...
    async query(request: QueryRequest): Promise<QueryResponse> {
        ...
        try {
            ...
            const relevantResults = searchResults.filter(result => result.score >= minScore);
            ...
            const tokensUsed = this.estimateTokens(request.query, relevantResults, answer);
    ...
    private buildFilter(scope?: QueryRequest['scope']): Record<string, any> | undefined {
...
Please provide a detailed answer based on the code above.`;
    ...
    private buildSourceReferences(results: SearchResult[]): SourceReference[] {
        return results.map(result => ({
            ...
            chunk: result.content.substring(0, 200) + (result.content.length > 200 ? '...' : ''),
    ...
    private estimateTokens(query: string, results: SearchResult[], answer: string): number {
